...
fn ddc_hint() -> Option<String> {
    ...
    if !readable {
        return Some(
            "Falta el módulo i2c-dev: sin él no se puede ajustar el brillo por DDC/CI.".to_string(),
        );
    }
        ...
        match ddcutil(&["detect", "--brief"]) {
            Ok(detected) => {
                for (connector, display) in parse_ddcutil_detect(&detected) {
                    ...
                    let percent =
                        ddcutil(&["--display", &display, "getvcp", VCP_BRIGHTNESS, "--brief"])
                            .ok()
...
"#;
    ...
    fn refuses_to_invent_a_brightness() {
        ...
        assert_eq!(
            parse_vcp_brightness("VCP 10 C 45 0"),
            None,
            "no dividing by zero"
        );
    }
